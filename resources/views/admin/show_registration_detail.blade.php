...
@endif
    ...
    <div class="container block mb-4 mt-3">
        ...
        <h4 class="ml-3 pt-3 text-center" style="color:#000; font-family:Silka-Black; letter-spacing:1px">
            <b>Sku Registration Details</b></h4>
                            ...
                            <table class="table table-striped">
                                <thead>
                                    <tr>
                                        ...
                                        <th scope="col">Vendor Entity Name</th>
                                ...
                                <tbody>
                                    ...
                                    @foreach ($registration as $item)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{url('view-sku-vendor-entity-detail/'.$item->id)}}" class="btn-submit"><b>View Product</b></a>
                                            </td>
                                            <td>
                                                @if ($item->status == 'Pending')
                                                    <a href="" class="btn-submit btn-pending" data-toggle="modal"
                                                        data-target="#updateModal{{ $item->id }}"><b>{{ $item->status }}</b></a>
                                                @elseif($item->status == 'Approved')
                                                    <a href="" class="btn-submit btn-approved" data-toggle="modal"
                                                        data-target="#updateModal{{ $item->id }}"><b>{{ $item->status }}</b></a>
                                                @endif
                                            </td>
                                        </tr>
                ...
                <div class="modal-body">
                    <form action="update-sku-registration-status" method="POST">
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="sku_status" id="inlineRadio1"
                                value="Pending" {{ $item->status == 'Pending' ? 'checked' : '' }}>
                            <label class="form-check-label" for="inlineRadio1">Pending</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="sku_status" id="inlineRadio2"
